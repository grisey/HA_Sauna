...
def fault_message(key, value):
    ...
    if key in ROLES:
        detail = {"measurement_stale": "Der letzte Messwert ist veraltet.",
            "validity_unconfigured": "Das Höchstalter eines Messwerts ist noch nicht eingestellt."}.get(value,
            "Zurzeit ist kein gültiger Messwert verfügbar.")
        return ROLES[key] + ": " + detail
    if key == "cooling_light":
        return {"brightness_required": "Die Lichthelligkeit für die Zwangskühlung ist noch nicht eingestellt.",
                "restore_failed": "Die vorherige Lichthelligkeit konnte nicht wiederhergestellt werden."}.get(value,
                "Das Saunalicht konnte nicht eingestellt werden.")
    text = FAULTS.get(key, "Eine Störung wurde erkannt. Bitte die Gerätediagnose prüfen.")
    if value == "pending":
...
def issues(runtime):
    ...
    for key, value in (device.faults.items() if device else ()):
        ...
        if value == "validity_unconfigured" or (key == "regulation_temperature_unavailable"
                and "sensor_timeout_seconds" in device.missing_configuration):
            continue
        result.append({"key": key, "message": fault_message(key, value),
...
def parameter_error(error):
    ...
    detail = {"required": "Bitte einen Wert eingeben.", "invalid_number": "Bitte eine gültige Zahl eingeben.",
        "positive": "Der Wert muss größer als null sein.", "non_negative": "Der Wert darf nicht negativ sein.",
        ...
        "window_not_divisible": "Das Zeitfenster muss durch den Zeitabstand der Personenprüfung teilbar sein."}.get(error.code,
