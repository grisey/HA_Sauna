...
class SaunaPanel extends HTMLElement {
  ...
  drawSettings() {
    ...
    for(const d of state.parameters){const input=this.$(`#parameters input[name="${d.key}"]`);input.disabled=!this.hass.user?.is_admin||(state.configuration_locked&&!d.live_editable);if(!input.dataset.edited&&this.shadowRoot.activeElement!==input)input.value=d.key==="target_temperature_c"?state.target_temperature:(state.configuration.parameters[d.key]??"");}
