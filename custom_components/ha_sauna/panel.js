...
const phases = {aus:"Aus",aufheizen:"Aufheizen",bereit:"Bereit",saunagang:"Saunagang",nachlauf:"Ofenkühlung",zwangskühlung:"Zwangskühlung (historisch)",manuell:"Manuell"};
...
class SaunaPanel extends HTMLElement {
  ...
  drawCurrent() {
    ...
    const availabilityCard=s.operation_enabled&&availability&&availability.gang_elapsed_seconds==null?(
      availability.minimum_wait_seconds>0?`<aside class="availability-card wait" aria-label="Startmöglichkeit"><small>Nächster Start frühestens in</small><strong>${roughMinutes(availability.minimum_wait_seconds,"ceil")}</strong><small class="availability-note">Die Temperaturbereitschaft wird danach erneut geprüft.</small></aside>`
        ...
        :availability.until_ready_seconds===0?`<aside class="availability-card ready" aria-label="Bereitschaft"><small>Bereitschaft</small><strong>Jetzt bereit</strong></aside>`
        :""
    ):"";
    ...
    const deadlineLabels={confirmation:"Aufgussbestätigung",session_gap:"Sitzungsende"};
    ...
    if(session?.after_run){
      ...
      timerRows.push([phase.paused_at?"Ofenkühlung pausiert":"Ofenkühlung",duration(phaseRemaining(phase))]);
    }
    ...
    if(s.phase_timer&&!new Set(["after_run","session_light","session_gap"]).has(s.phase_timer.kind))timerRows.push([s.phase_timer.label,`<span data-phase-timer="${esc(s.phase_timer.kind)}">${duration(s.phase_timer.seconds)}</span>`,true]);
    ...
    const manualPhase=session?.after_run?{purpose:"after_run",token:session.after_run.phase_id,label:"Ofenkühlung jetzt beenden"}:null;
    const finishPhase=manualPhase?`<div class="row"><button data-action="end-phase:${manualPhase.purpose}:${esc(encodeURIComponent(manualPhase.token))}" ${disabled?"disabled":""}>${manualPhase.label}</button></div>`:"";
    const temperatureColor={aufheizen:"#e58a55",bereit:"#4f9a6a",saunagang:"#cf7447",nachlauf:"#5b8ca8"}[s.phase]||"#888";
    ...
    const restartThreshold=s.thermostat_target==null?null:s.thermostat_target-(p.readiness_hysteresis_c??0);
  ...
  presenceDetails() {
    const p=this.state?.presence, rules=this.state?.rule_inputs;
    if(!p&&!rules)return "";
    const report=r=>!r?"Noch keine Meldung":`${r.available?({present:"Anwesend",absent:"Abwesend",unknown:"Unbekannt"}[r.occupancy]||"Unbekannt"):"Nicht verfügbar"} · ${r.assertion==="provisional_proxy"?"Vorläufiger Proxy":r.assertion==="proxy_retraction"?"Proxy zurückgenommen; keine beobachtete Abwesenheit":"Direkte Präsenz"} · Ereignis ${when(r.effective_at)} · empfangen ${when(r.received_at)}${r.reason?` · ${r.reason}`:""}`;
    return `<div class="card"><h2>Präsenz und Regelursache</h2><dl><dt>Gewünschte Quelle</dt><dd>${p?.configured_source==="ha_presence"?"Externe Präsenz (vorbereitet)":"Proxy"}</dd><dt>Wirksame Quelle</dt><dd>Proxy</dd><dt>Aktuelle Belegung</dt><dd>${esc(report(p?.current))}</dd>${Object.entries(p?.external||{}).map(([source,r])=>`<dt>${esc(source)}</dt><dd>${esc(report(r))}</dd>`).join("")}<dt>Gang-Abschaltveto</dt><dd>${rules?.gang_veto?"Freigegeben; aktuelle Wirkung siehe Regelursache":"Inaktiv"}</dd><dt>Türanforderung</dt><dd>${rules?.door_request?"Einmalige Anforderung liegt an":"Keine aktuelle Anforderung"}</dd><dt>Ofenkühlung</dt><dd>${rules?.cooling?"Aktiv":"Inaktiv"}</dd></dl><p class="muted">Externe Präsenz wird beobachtet und ersetzt den Proxy noch nicht. Die Aktivierungsregeln sind offen. Das optionale Audioziel ist vorbereitet; es findet keine Wiedergabe statt.</p></div>`;
  }
  ...
  chart(records,session,gangs) {
    ...
    const projection=this.shown?.phase_projection;
    if(projection){
      for(const p of projection.intervals||[]){
        const gang=gangs.find(g=>g.gang_id===p.source_id);
        const klass=p.phase==="saunagang"?(gang&&!gang.infusion_events.length?"gang provisional":"gang"):(styles[p.phase]||"phase-other");
        svg+=interval(p.started_at,p.ended_at,klass,`${p.phase}${p.complete===false?" · unvollständig":""}`);
      }
    }else phases.forEach((p,i)=>{if(styles[p.payload.phase])svg+=interval(p.received_at,phases[i+1]?.received_at,styles[p.payload.phase],p.payload.phase);});
    const doorEvents=session.timeline.processed.filter(e=>e.kind==="door_open"||e.kind==="door_close");
    if(!projection)for(const e of session.timeline.processed.filter(e=>e.kind==="ventilation_confirmed")){const close=doorEvents.find(d=>d.kind==="door_close"&&stamp(d.effective_at)>=stamp(e.effective_at));svg+=interval(e.effective_at,close?.effective_at,"vent","lüften");}
    if(!projection)svg+=gangs.map(g=>interval(g.started_at,g.ended_at,g.infusion_events.length?"gang":"gang provisional",`Gang ab ${when(g.started_at)}`)).join("");
  ...
  drawDiagnostics() {
    ...
    let html='<details><summary>Hinweise zur Erkennung</summary><p class="muted">Die Kurven zeigen die gespeicherten Erkennungswerte, die gestrichelten Linien die zugehörigen Schwellen. Ein Grenzübertritt löst erst dann ein Ereignis aus, wenn auch die übrigen Bedingungen erfüllt sind. Nach Beginn eines Gangs und während der Ofenkühlung ruht die Personensuche. Weitere Aufgüsse werden im Gang weiter erkannt.</p></details>';
