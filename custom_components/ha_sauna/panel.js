...
class SaunaPanel extends HTMLElement {
  ...
  drawCurrent() {
    ...
    const energyLabel={measured:"Aus gemessener Leistung",estimated:"Geschätzt",mixed:"Messung mit geschätzten Anteilen",incomplete:"Unvollständig"};
    const energyText=`${(s.energy_kwh??0).toLocaleString("de-DE",{maximumFractionDigits:3})} kWh · ${energyLabel[s.energy_source]||"Geschätzt"}`;
    const heatCaption=s.heating_feedback===true?(s.heating_observation?.estimated?"Schütz ein":"Heizt"):s.heating_feedback===false?"Ofen aus":"Unbekannt";
    const heatSource={power:"Leistung gemessen",independent_feedback:"Unabhängig rückgemeldet",contactor:"Nach Schützstellung geschätzt",unknown:"Rückmeldung fehlt"}[s.heating_observation?.source]||"Rückmeldung fehlt";
  ...
  drawHistory() {
    ...
    const e=session.energy; const energySummary=e?`<p class="muted" data-history-energy>Sessionenergie: ${(e.measured_kwh+e.estimated_kwh).toLocaleString("de-DE",{maximumFractionDigits:3})} kWh · ${e.unknown_seconds?"Unvollständig":e.measured_seconds?(e.estimated_seconds?"Messung mit geschätzten Anteilen":"Aus gemessener Leistung"):"Geschätzt"}</p>`:"";
