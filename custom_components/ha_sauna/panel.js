...
class SaunaPanel extends HTMLElement {
  ...
  drawCurrent() {
    ...
    const fiveMinuteEstimate=seconds=>{
      ...
      if(minutes<5)return "unter 5 min";
      return `${Math.max(5,Math.round(minutes/5)*5)} min`;
    };
    const startWindow=seconds=>{
      const minutes=Math.max(0,Number(seconds)||0)/60;
      if(minutes<5)return "unter 5 min";
      return `${Math.floor(minutes/5)*5} min`;
    };
    const timerLine=s.phase_timer;
    const phaseTimerText=s.operation_enabled&&timerLine?({
      gang:`seit ${duration(timerLine.seconds)}`,
      after_run:`${timerLine.mode==="paused"?"pausiert – noch":"noch"} ${duration(timerLine.seconds)}`,
      cooling:`noch ${duration(timerLine.seconds)}`,
    }[timerLine.kind]||""):"";
    const blockerText=s.operation_enabled&&availability?.blocker&&!phaseTimerText?(
      availability.minimum_wait_seconds>0?`Start gesperrt – noch ${duration(availability.minimum_wait_seconds)}`:(availability.message||"Start derzeit gesperrt.")
    ):"";
    const readinessText=s.operation_enabled&&s.phase==="aufheizen"&&availability&&!availability?.blocker&&availability.until_ready_seconds>0&&availability.ready_estimated
      ?`noch ${fiveMinuteEstimate(availability.until_ready_seconds)} bis bereit`
      :s.operation_enabled&&s.phase==="bereit"&&availability&&!availability?.blocker&&availability.until_ready_seconds===0&&availability.start_window_seconds>0
        ?`noch ${startWindow(availability.start_window_seconds)}`
        :"";
    const availabilityLine=phaseTimerText||blockerText||readinessText;
    const availabilityText=s.operation_enabled&&availability&&!availabilityLine&&availability.until_ready_seconds!==0?(availability.message||"Startzeit noch nicht abschätzbar."):"";
    ...
    const phaseLabel=s.phase==="aufheizen"?"Heizen":phases[s.phase]||"Unbekannt";
    const stateLine=`<div class="state-line"><div class="state-summary"><strong class="phase" data-phase="${esc(s.phase)}">${phaseLabel}</strong>${availabilityLine?`<span class="availability-line ${availability?.blocker?"wait":""}">${esc(availabilityLine)}</span>`:""}<span class="badge">${count} ${count===1?"Saunagang":"Saunagänge"}</span></div></div><p id="phase-detail" class="muted">${esc(activity)}</p>`;
