...
class SaunaPanel extends HTMLElement {
  ...
  drawCurrent() {
    ...
    const lightPresetButtons=canControl=>`<button data-action="light:false" ${canControl?"":"disabled"}>Aus</button><button data-action="light:normal" ${canControl?"":"disabled"}>Normallicht</button><button data-action="light:true" ${canControl?"":"disabled"}>Raumlicht · ${num(p.session_light_brightness_percent,0)} %</button><button data-action="light:auto" ${canControl?"":"disabled"}>Automatik · ${automatic}</button>`;
    const lightControls=`<div class="light-controls"><small>Licht</small><div class="row">${lightPresetButtons(permissions.light)}</div></div>`;
  ...
  hoverChart(e) {
    ...
    const rows=[];for(const p of this.positions)for(const q of ["temperature","humidity"]){
      ...
      if(nearest&&(!ttl||Math.abs(stamp(nearest.received_at)-time)<=ttl))rows.push(`<span style="color:${q==="temperature"?'#e25d40':'#2f8bde'}">${q==="temperature"?"Temperatur":"Luftfeuchte"}${this.historyDetail?` ${p==="upper"?"oben":"unten"}`:""}</span><br>${num(nearest.value,3)} ${q==="temperature"?"°C":"%"} · ${when(nearest.received_at)}`);
    }
