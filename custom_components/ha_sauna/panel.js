...
class SaunaPanel extends HTMLElement {
  ...
  drawSettings() {
    ...
    if(this.settingsEntry!==this.entry){
      ...
      const isLegacyProgramField=d=>/^program_[12]_(start_c|end_c|gangs)$/.test(d.key);
      const fields=Object.entries(groups).map(([key,title],index)=>`<details class="settings-group" ${index===0?"open":""}><summary>${title}</summary><div class="forms">${state.parameters.filter(d=>d.group===key&&!d.expert&&!isLegacyProgramField(d)).map(field).join("")}</div></details>`).join("");
