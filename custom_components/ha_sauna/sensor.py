...
class SaunaPhase(SaunaEntity, SensorEntity):
    ...
    def extra_state_attributes(self):
        ...
        return {
            ...
            "after_run_ends_at": session.after_run.ends_at.isoformat() if session and session.after_run and session.after_run.ends_at else None,
            "after_run_paused": bool(session and session.after_run and session.after_run.paused_at),
            "after_run_remaining_seconds": session.after_run.remaining_seconds if session and session.after_run else None,
