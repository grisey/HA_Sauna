...
def advance(state, at, *, power_w, valid_until, heating, nominal_kw):
    ...
    return replace(state, accounted_at=at,
        ...
        estimated_kwh=state.estimated_kwh + (nominal_kw * remaining / 3600 if heating is True else 0),
        measured_seconds=state.measured_seconds + measured,
        estimated_seconds=state.estimated_seconds + (remaining if heating is not None else 0),
