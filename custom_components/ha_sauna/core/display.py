...
def phase_timer(controller, now):
    ...
    if session.after_run:
        ...
        if phase.paused_at is not None:
            return {"kind": "after_run", "label": "Nachlauf pausiert, noch",
                    "seconds": phase.remaining_seconds, "mode": "paused"}
        return remaining("after_run", "Nachlauf noch", phase.ends_at)
    if session.cooling and session.cooling.ends_at and session.cooling.paused_at is None:
...
def start_availability(controller, now, temperature_rate=None):
    ...
    if active is not None:
        result.update(blocker={"kind": "gang"},
    ...
    valid_temperature = (not isinstance(temperature, bool)
                         and isinstance(temperature, (int, float)) and isfinite(temperature))
    valid_target = (not isinstance(target, bool)
    ...
    if controller.protection:
        result.update(blocker={"kind": "protection", "reasons": tuple(sorted(controller.protection))},
                      message="Ein Saunagang ist wegen einer Schutzsperre derzeit nicht möglich.")
        return result
    if controller.inhibits:
        result.update(blocker={"kind": "inhibit", "reasons": tuple(sorted(controller.inhibits))},
                      message="Ein Saunagang ist derzeit gesperrt.")
        return result
    if not valid_target:
        result.update(blocker={"kind": "temperature_configuration"},
                      message="Die Regeltemperatur ist noch nicht verfügbar.")
        return result
    if not valid_temperature:
        result.update(blocker={"kind": "temperature_unavailable"},
    ...
    if after_run is not None:
        after_seconds = (after_run.remaining_seconds if after_run.paused_at is not None
                         else max(0, (after_run.ends_at - now).total_seconds()))
        if after_run.paused_at is not None:
            result.update(blocker={"kind": "after_run_paused", "seconds": after_seconds},
        ...
        if cooling is not None:
            ...
            following_cooling_seconds = max(0, cooling.remaining_seconds - prospective_credit)
        result.update(minimum_wait_seconds=after_seconds + following_cooling_seconds,
            blocker={"kind": "after_run", "seconds": after_seconds,
    ...
    if cooling is not None and cooling.started_at is not None and cooling.paused_at is None:
        cooling_seconds = cooling.remaining_seconds
        result.update(minimum_wait_seconds=cooling_seconds,
    ...
    if cooling is not None and cooling.paused_at is not None:
        result.update(blocker={"kind": "cooling_paused"}, pending_cooling=True,
    ...
    if wait_seconds is not None:
        ...
        result["message"] = "Der nächste Saunagang hängt noch von der laufenden Erkennung ab."

    ready = (session.ready_at is not None and temperature is not None and target is not None
             and temperature >= target - controller.parameters.values["readiness_hysteresis_c"])
    if ready:
        result["until_ready_seconds"] = 0
        result["start_window_seconds"] = max(0, controller.heating_limit_seconds - session.heating.elapsed_seconds)
        if result["blocker"] is None:
            result["message"] = "Bereit für einen Saunagang."
        return result

    valid_rate = (not isinstance(temperature_rate, bool)
                  and isinstance(temperature_rate, (int, float))
                  and isfinite(temperature_rate) and temperature_rate > 0)
    if valid_rate and valid_temperature and valid_target:
        result.update(until_ready_seconds=max(0, (target - temperature) / temperature_rate),
            ready_estimated=True)
        if result["blocker"] is None:
            result["message"] = "Aufheizzeit wird aus dem belastbaren Temperaturtrend geschätzt."
    elif result["blocker"] is None:
        result["message"] = "Noch nicht abschätzbar: Es fehlt ein belastbarer positiver Temperaturtrend."
    return result
