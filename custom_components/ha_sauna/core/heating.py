...
def advance(state: HeatingTime, at, reset_seconds: float) -> HeatingTime:
    ...
    return replace(state, elapsed_seconds=elapsed, accounted_at=at, last_reset_at=reset_at)


def report(state: HeatingTime, heating: bool | None, at, reset_seconds: float) -> HeatingTime:
    ...
    if state.reported_heating is True:
        intervals = intervals[:-1] + (replace(intervals[-1], ended_at=at,
            end_reason="feedback_off" if heating is False else "feedback_unknown"),)
    if heating is True:
        intervals += (HeatingInterval(at),)
    return replace(state, reported_heating=heating, off_since=at if heating is False else None,
                   intervals=intervals)
