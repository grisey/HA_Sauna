        ...
        if before == after and not changed - LIVE_TEMPERATURE_KEYS:
            async with runtime._lock:
                await apply_temperature_parameters(runtime, updated.parameters,
                    ...
                    program_mode=(after_program_mode if before_program_mode != after_program_mode else None),
                    new_program=before_program_mode != after_program_mode)
                if before_button_program != after_button_program:
                    runtime.configuration = replace(runtime.configuration,
                        button_program=after_button_program)
                runtime.set_log_level(updated.log_level)
            return
        if runtime.session:
