...
class BrowserTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_overview_timers_stable_controls_german_settings_and_logging(self):
        ...
        await self.panel.locator('#current [data-action="operation"]').click()
        identity=self.runtime.session.session_id
        await self.panel.locator('#target').fill("80")
        await self.panel.locator('[data-action="target"]').click()
        await expect(self.panel.locator('#target')).to_have_value("80")
        await expect(self.panel.locator('#current')).to_contain_text("Steigerung bis 86 °C")
        await self.panel.locator('#progression-step').fill("2")
        await self.panel.locator('[data-action="progression"]').click()
        await expect(self.panel.locator('#progression-step')).to_have_value("2")
        ...
        await expect(self.panel.locator('input[name="temperature_increase_c"]')).to_be_enabled()
