...
// frontend profile list or a supposed maximum number of sauna rounds.
{
  ...
  assert.equal(
    ...
    "82 → 99 °C",
    "one distribution step still reaches the configured end target",
  );
  assert.equal(
    p.programSteps({ temperature_steps: [80, 86, 90] }),
    "80 → 86 → 90 °C",
    "manual stages remain exact",
  );
  assert.deepEqual(
    JSON.parse(JSON.stringify(p.distributedSteps(80, 90, 9))),
    [],
    "stage expansion observes the configured maximum",
  );
}
...
// array merely because the user chose "Individuell".
{
  const calls = [],
    inputs = {
      "#progression-start": { value: "80" },
      "#progression-end": { value: "95" },
      "#progression-gangs": { value: "4" },
    };
  let release;
  ...
  saved
    .then(() => {
      assert.deepEqual(JSON.parse(JSON.stringify(calls)), [
        ["/entry-1/temperature", "POST", { final_temperature_c: 95 }],
      ]);
...
// the exact stage array accepted by the backend.
{
  ...
  p.action("program-apply")
    .then(() => {
      assert.deepEqual(JSON.parse(JSON.stringify(calls)), [
        ["/entry-1/program", "POST", { temperature_steps: [80, 86, 90] }],
      ]);
...
for (const selected of [null, "quiet"]) {
  ...
  const p = Object.assign(Object.create(Panel.prototype), {
    ...
    state: {
      ...
      configuration: {
        program_mode: selected ? "progressive" : "constant",
    ...
    .then(() => {
      assert.deepEqual(JSON.parse(JSON.stringify(calls)), [
        [
          "/entry-1/program",
          "POST",
          { target_temperature_c: 80, final_temperature_c: 95, temperature_gangs: 4 },
        ],
      ]);
...
// This covers the interval refresh after adding a row and after a rejected save.
{
  ...
  p.savePrograms()
    ...
    .then(() => {
      assert.equal(
        p.programDraft[0].name,
        "Nach Fehler",
        "rejected save retains the submitted draft",
      );
      p.renderProgramLibrary();
      assert.equal(
        library.innerHTML,
...
// share the program permission/session lock.
{
  ...
  const p = Object.assign(Object.create(Panel.prototype), {
    ...
    $: (selector) =>
      ({ "#button-settings": button, "#program-library": library, ...inputs })[
        selector
      ] || null,
  });
  p.drawButtonProgram();
  ...
  p.action("button-program")
    .then(async () => {
      ...
      p.drawButtonProgram();
      p.programLibraryNeedsRender = true;
      p.renderProgramLibrary();
...
// A normal user reaches the same Programs and Saunataster cards, without the
// administrator's technical settings and archive controls.
{
